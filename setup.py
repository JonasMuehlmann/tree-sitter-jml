...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_jml", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_jml": ["*.pyi", "py.typed"],
        "tree_sitter_jml.queries": ["*.scm"],
    },
    ext_package="tree_sitter_jml",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_jml/binding.c",
                "src/parser.c",
            ],
            define_macros=[
